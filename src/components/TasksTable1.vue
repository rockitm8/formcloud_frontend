<template>
  <div class="tasks-table">
    <div class="container">
      <JlTable :url="datatable.url" :columns="datatable.columns" />
    </div>
  </div>
</template>
...
import JlTable from 'jl-datatable';
...
export default {
  ...
  data() {
    return {
      ...
      task_detail: {},
      datatable: {
        url: `${this.$store.state.backend_url}/api/tasks/`,
        columns: [
          {
            title: 'Task Name',
            key: 'task_name',
  ...
  methods: {
    bodyClicked() {
      // const row = this.dataTable.row().data();
      // console.log('row:', row);
      // this.$router.push({ path: `/task-info/${row.id}/` });
    },
    ...
    //       { data: 'went_manually' },
    //     ];
    //     this.dataTable = $('#filtertable').DataTable({
